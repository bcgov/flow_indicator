...
-   **Lack of change in annual averages explained by monthly flow trends.** Trends in median monthly flows are increasing for winter and spring months (November - May), however summer months (June - September) are decreasing for many rivers. A similar pattern can be seen for low flows.
...
of freshet as the date whereby 50% of river discharge for that water year has
taken place.
...
stations identified increasing average annual flows, of which
...
displayed decreasing average annual flows, while the remaining
...
`r Low_cat$n[Low_cat$category == "> 5% decrease"]` were at a rate greater than
5% per decade.
...
Trends in the timing of low flow are far less clear than that of timing of freshet. Trends indicating earlier timing of low flows were present in some hydrologic zones, most prominent in the Southern Quesnel Highland (`r date_low_by_zone_direction$n[date_low_by_zone_direction$HYDZN_NAME == "Southern Quesnel Highland" & date_low_by_zone_direction$direction == "negative"]` of `r sum(date_low_by_zone_direction$n[date_low_by_zone_direction$HYDZN_NAME == "Southern Quesnel Highland"])` stations), the Fraser Plateau (`r date_low_by_zone_direction$n[date_low_by_zone_direction$HYDZN_NAME == "Fraser Plateau" & date_low_by_zone_direction$direction == "negative"]` of `r sum(date_low_by_zone_direction$n[date_low_by_zone_direction$HYDZN_NAME == "Fraser Plateau"])` stations) and Nechako Plateau (`r date_low_by_zone_direction$n[date_low_by_zone_direction$HYDZN_NAME == "Nechako Plateau" & date_low_by_zone_direction$direction == "negative"]` of `r sum(date_low_by_zone_direction$n[date_low_by_zone_direction$HYDZN_NAME == "Nechako Plateau"])` stations).
...
-   ^1^[Water Year](https://water.usgs.gov/nwc/explain_data.html)

-   ^2^[Low Flow Year](https://www.epa.gov/hydrowq/definition-and-characteristics-low-flows)
