...
Average annual flow was calculated within each water year (Octboer - October). The mean daily flow was identified for each station and each year of available data, and
...
## Volume of peak flow and freshet
...
## Timing of yearly data
- Water Year = October - October
- Summer low flow = May - October

## Filtering procedures

1) Missing data (no mention of if value is estimated in hydat dataset)

- Calculate percentage of days with an NA for each station and each year (perc_daily_missing). Identify "incomplete years", which is a year with > 1 missing value. 
Remove any years with missing data (even if just a single day) that totals more than 30%.

- Remove stations with less than 10 years of data and stations that did not have data in 2019.

- Identified gaps in data and removed data prior to big data gaps (this could be improved).

2) Duplicated
- 121 stations are duplicated i.e. more than one station on a river. Used filter table to remove upstream stations (52 of 121 stations removed)

3) Regulated
- Regulated stations identified and filtered based on degree of regulation (21 stations removed)
