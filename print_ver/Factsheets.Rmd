...
variable_label_df = data.frame(
  varname = c('Median','Min_7_Day','Min_30_Day','Min_7_Day_DoY','DoY_50pct_TotalQ'),
  labelname = c('Median Flow','7-Day Low Flow','30-Day Low Flow','7-Day Low Flow Day-of-year','50% Annual Flow Day-of-year')
)
...
most_recent_data = paste(
    flow_dat %>% filter(Year == max(flow_dat$Year)) %>% select(Month) %>% distinct() %>% tail(1),
  max(flow_dat$Year)
)

# Oldest data.
oldest_data = paste(
  flow_dat %>% filter(Year == min(flow_dat$Year)) %>% select(Month) %>% distinct() %>% slice(2),
  min(flow_dat$Year)
)
```

```{r add_most_recent_median_flow_to_stations}
stations = stations %>% 
  left_join(flow_dat %>% 
              filter(!is.na(Median),
                     Month == 'All') %>% 
              ...
              slice_max(Year) %>% 
              ungroup() %>% 
              dplyr::select(STATION_NUMBER,Median,Year_of_Median_Flow = Year)) 
```
...
make_leaflet = function(station, stations){
  ...
  stations_to_not_label = map(stations_with_labels$data, ~ {
    ...
    #   as.data.frame() %>% 
      ...
      # Add median flow of last year for focal station
      mutate(focal_st_flow = stations_with_labels[stations_with_labels$STATION_NUMBER == .[.$dist_mat == 0,]$other_station,]$Median) %>% 
      # Add median flow of last year for each nearby 'other station'
      left_join(stations_with_labels %>% st_drop_geometry() %>% dplyr::select(other_station = STATION_NUMBER, other_flow = Median),
  ...
  stations_marked = stations_marked %>% 
    mutate(size_bins = cut(Median,number_size_classes,labels = F))
  
  size_legend_labels = tibble(
    range = cut(stations_marked$Median,number_size_classes, dig.lab = 4) %>% 
        levels() %>% 
        str_remove_all('\\(|\\]') %>% 
        str_replace_all(',',' - ') %>% 
  ...
  addProviderTiles(leaflet_provider_choice) %>%
    ...
    addLegendSize(
      title = HTML('Median Flow (m<sup>3</sup>/s)'),
      color = 'grey',
...
make_summary_table = function(dat, station){
  ...
  summary_most_recent_year = dat %>% 
    ...
    mutate(across(c(Median,Min_7_Day,Min_30_Day), ~ paste0(round(.x,1),' m^3/s'))) %>% 
    dplyr::select(`Station Name`,`Station ID Number`,
                  `Years of Data`,Coordinates,Region,
                  `Other Stations within x`,
                  Median,Min_7_Day,Min_30_Day) %>% 
    pivot_longer(cols = everything(), names_to = 'Parameter', values_to = 'Median Value') %>% 
...
make_data_histogram = function(dat){
  ...
  data_histogram = dat %>% 
    filter(Month != 'All') %>%
    count(Year,Month, name = 'data_month') |> 
...
for(i in 1:nrow(stations)){
  ...
  median_flow_linegraph = make_linegraph(dat = this_dat, variable_to_plot = 'Median')
  ...
  min_7day_doy_circle_plot = make_day_of_year_circle_plot(this_dat, variable_to_plot = 'Min_7_Day_DoY')
  
  # halfflow_doy_circle_plot = make_day_of_year_circle_plot(this_dat, variable_to_plot = 'DoY_50pct_TotalQ')
  # Compose elements===================================
  
  col_one = ggarrange(
    summary_table_plot,
    min_7day_doy_circle_plot,
    nrow = 2
  )
  
  col_two = ggarrange(
    median_flow_linegraph,
